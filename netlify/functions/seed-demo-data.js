...
// ─── Helpers (defined at top level to avoid hoisting issues) ───
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ─── Step 3: Insert 50 Medications (BULK) ───
    const medications = [
    ...
    // BULK insert all 50 medications in one query
    {
      const values = [];
      const params = [];
      medications.forEach((med, i) => {
        ...
        params.push(med.id, med.name, med.strength, med.form, med.min, med.ipb);
      });
      await client.query(
    ...
    const batchData = []; // { medicationId, batchCode, expiryStr, brand, ipb, serial, medName, locs }
    let batchCounter = 0;
    ...
    for (const med of medications) {
      ...
      for (let b = 0; b < numBatches; b++) {
        ...
        batchData.push({
          medicationId: med.id, batchCode, expiryStr, brand,
          ipb: med.ipb, serial: `SN-${batchCode}`,
          medName: `${med.name} ${med.strength}`, locs: med.locs,
        });
      }
    }

    // BULK insert all batches and get IDs back
    {
      const values = [];
      const params = [];
      batchData.forEach((b, i) => {
        const off = i * 6;
        values.push(`($${off+1}, $${off+2}, $${off+3}, $${off+4}, $${off+5}, $${off+6})`);
        params.push(b.medicationId, b.batchCode, b.expiryStr, b.brand, b.ipb, b.serial);
      });
      ...
      rows.forEach((row, i) => { batchData[i].id = row.id; });
    }

    // ─── Step 5: Distribute Inventory (BULK) ───
    ...
    for (const batch of batchData) {
      ...
      for (const locId of selectedLocs) {
        ...
        inventoryRecords.push({
          batchId: batch.id, locationId: locId, onHand,
          medId: batch.medicationId, medName: batch.medName,
          batchCode: batch.batchCode, ipb: batch.ipb,
        });
      }
    }

    // BULK insert inventory in chunks of 200
    for (let c = 0; c < inventoryRecords.length; c += 200) {
      const chunk = inventoryRecords.slice(c, c + 200);
      const values = [];
      const params = [];
      chunk.forEach((inv, i) => {
        const off = i * 3;
        values.push(`($${off+1}, $${off+2}, $${off+3})`);
        params.push(inv.locationId, inv.batchId, inv.onHand);
      });
      await client.query(
    ...
    // ─── Step 6: Set location-specific min levels (BULK) ───
    {
      const minLevelData = [];
      for (const med of medications) {
      ...
      for (let c = 0; c < minLevelData.length; c += 200) {
        const chunk = minLevelData.slice(c, c + 200);
        const values = [];
        const params = [];
        chunk.forEach((ml, i) => {
          const off = i * 4;
          values.push(`($${off+1}, $${off+2}, $${off+3}, $${off+4})`);
          params.push(ml.medId, ml.locId, ml.locMin, new Date('2026-02-18T09:00:00Z'));
        });
        await client.query(
    ...
    // Generate medication_created activity for first days
    ...
    for (const med of medications) {
      ...
      activityLogs.push({
        userId: user.id, actionType: 'medication_created', entityType: 'medication',
        entityId: med.id, locationId: null,
    ...
    while (currentDay <= endDate) {
      ...
      const isDeliveryDay = dayOfWeek === 1 || dayOfWeek === 3 || dayOfWeek === 5;
      ...
      for (let i = 0; i < numLogins; i++) {
        ...
        activityLogs.push({
          userId: user.id, actionType: 'login', entityType: null, entityId: null,
          locationId: user.location || null, details: { username: user.username },
          occurredAt: workingHourTime(currentDay),
        });
      }
      ...
      for (let i = 0; i < numDispensing; i++) {
        ...
        transactions.push({ userId: user.id, locationId: inv.locationId, batchId: inv.batchId, medicationId: inv.medId, delta, reason, type: 'out', occurredAt });
        activityLogs.push({ userId: user.id, actionType: 'stock_out', entityType: 'medication', entityId: inv.medId, locationId: inv.locationId, details: { medicationName: inv.medName, batchId: inv.batchId, batchCode: inv.batchCode, delta, reason }, occurredAt });
      }
      ...
      if (isDeliveryDay) {
        ...
        for (let i = 0; i < numDeliveries; i++) {
          ...
          const occurredAt = workingHourTime(currentDay);
          transactions.push({ userId: user.id, locationId: inv.locationId, batchId: inv.batchId, medicationId: inv.medId, delta, reason, type: 'in', occurredAt });
          activityLogs.push({ userId: user.id, actionType: 'stock_in', entityType: 'medication', entityId: inv.medId, locationId: inv.locationId, details: { medicationName: inv.medName, batchId: inv.batchId, batchCode: inv.batchCode, delta, boxes, reason }, occurredAt });
        }
      }
      ...
      if (isWeekday && locIds.length >= 2) {
        ...
        for (let i = 0; i < numTransfers; i++) {
          ...
          const occurredAt = workingHourTime(currentDay);
          transactions.push({ userId: user.id, locationId: inv.locationId, batchId: inv.batchId, medicationId: inv.medId, delta: -qty, reason: `Transfer to ${targetLoc}`, type: 'out', occurredAt });
          transactions.push({ userId: user.id, locationId: targetLoc, batchId: inv.batchId, medicationId: inv.medId, delta: qty, reason: `Transfer from ${inv.locationId}`, type: 'in', occurredAt });
          activityLogs.push({ userId: user.id, actionType: 'stock_transfer', entityType: 'medication', entityId: inv.medId, locationId: inv.locationId, details: { medicationName: inv.medName, batchId: inv.batchId, batchCode: inv.batchCode, quantity: qty, sourceLocationId: inv.locationId, targetLocationId: targetLoc }, occurredAt });
        }
      }
    ...
    for (let c = 0; c < transactions.length; c += 200) {
      const chunk = transactions.slice(c, c + 200);
      const values = [];
      const params = [];
      chunk.forEach((t, i) => {
        const off = i * 8;
    ...
    // ─── Step 8: Generate Orders (BULK) ───
    const orderData = [];
    const numOrders = 20 + Math.floor(Math.random() * 11);
    for (let i = 0; i < numOrders; i++) {
      ...
      orderData.push({ med, user, urgency, status, orderedAt, quantity, fulfilledAt });
    }

    // Insert orders in one bulk query
    {
      const values = [];
      const params = [];
      orderData.forEach((o, i) => {
        const off = i * 10;
        values.push(`($${off+1}, $${off+2}, $${off+3}, $${off+4}, $${off+5}, $${off+6}, $${off+7}, $${off+8}, $${off+9}, $${off+10})`);
        params.push(
          o.med.id, o.user.id, o.quantity, o.urgency,
          o.status === 'fulfilled' ? 'Order completed' : o.urgency === 'emergency' ? 'Urgent clinical need' : 'Regular restock',
      ...
      // Add activity logs for orders
      orderData.forEach((o, i) => {
        activityLogs.push({
          userId: o.user.id, actionType: 'order_placed', entityType: 'medication', entityId: o.med.id,
          locationId: null,
          details: { medicationName: `${o.med.name} ${o.med.strength}`, quantity: o.quantity, urgency: o.urgency, orderId: rows[i].id },
          occurredAt: o.orderedAt,
        });
        if (o.status === 'fulfilled') {
          activityLogs.push({
            userId: o.user.id, actionType: 'order_fulfilled', entityType: 'medication', entityId: o.med.id,
            locationId: null,
            details: { medicationName: `${o.med.name} ${o.med.strength}`, quantity: o.quantity, orderId: rows[i].id },
    ...
    // min_level_changed logs
    for (let i = 0; i < 8; i++) {
      const med = pick(medications);
      const user = pick(pharmacists.length > 0 ? pharmacists : allStaff);
      const occurredAt = randomDate(new Date('2026-02-20'), new Date('2026-03-10'));
      activityLogs.push({
        userId: user.id, actionType: 'min_level_changed', entityType: 'medication', entityId: med.id,
    ...
    // ─── Step 9: BULK insert Activity Logs in chunks of 200 ───
    activityLogs.sort((a, b) => a.occurredAt - b.occurredAt);
    for (let c = 0; c < activityLogs.length; c += 200) {
      const chunk = activityLogs.slice(c, c + 200);
      const values = [];
      const params = [];
      chunk.forEach((log, i) => {
        const off = i * 7;
        ...
        params.push(
          log.userId, log.actionType, log.entityType || null,
          log.entityId ? String(log.entityId) : null,
          log.locationId || null, JSON.stringify(log.details || {}), log.occurredAt
        );
      });
      await client.query(
